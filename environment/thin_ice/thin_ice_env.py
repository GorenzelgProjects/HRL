...
class ThinIceEnv(gym.Env):
    ...
    def _load_data(self):
        ...
        self.teleporter_sprite_sheet = Spritesheet(teleporter_sprite_path, teleporter_xml_path)
        ...
        try:
            ...
            self.move_sound.set_volume(0.0 if self.headless else 0.1)  # Silent in headless
        except:
        ...
        try:
            moving_block_sound_path = _resolve_path("thin_ice/data/sound/movingBlockSound.ogg")
        ...
        try:
            all_tile_complete_sound_path = _resolve_path("thin_ice/data/sound/allTileComplete.ogg")
            ...
            self.all_tile_complete_sound = type('DummySound', (), {'play': lambda: None})()
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        # Mark exit
        if self.end_tile:
            if 0 <= self.end_tile.x < self.grid_width and 0 <= self.end_tile.y < self.grid_height:
                grid[self.end_tile.y, self.end_tile.x] = 5
        
        # Mark key
        if self.game.key and hasattr(self.game.key, 'x'):
            if 0 <= self.game.key.x < self.grid_width and 0 <= self.game.key.y < self.grid_height:
                grid[self.game.key.y, self.game.key.x] = 6
        
        # Mark keyhole
        if self.game.keyHole and hasattr(self.game.keyHole, 'x'):
            if 0 <= self.game.keyHole.x < self.grid_width and 0 <= self.game.keyHole.y < self.grid_height:
                grid[self.game.keyHole.y, self.game.keyHole.x] = 7
        
        # Mark player (overwrites other tiles)
        if self.player:
            if 0 <= self.player.x < self.grid_width and 0 <= self.player.y < self.grid_height:
    ...
    def _get_info(self) -> Dict:
        ...
        if self.player and self.end_tile:
            distance = abs(self.player.x - self.end_tile.x) + abs(self.player.y - self.end_tile.y)
    ...
    def render(self):
        ...
        if self.render_mode == "human":
            ...
            if len(self.all_sprites) == 0 and len(self.score_sprites) == 0:
                ...
                print("WARNING: No sprites to draw! This indicates a problem with sprite initialization.")
