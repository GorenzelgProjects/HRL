...
class AStarManager(BaseModelManager):
    def __init__(
        self, heuristic: str, partial_env: Callable[[int], gym.Env], save_dir: str
    ) -> None:
        self.heuristic = heuristic

        super().__init__(partial_env, model_name="astar", save_dir=save_dir)
    ...
    def _save_results(self, level_results: list[dict[Any, Any]]) -> None:
        ...
        output = {
            ...
            "summary": {
                ...
                "avg_plan_length": (
                    float(
                        np.mean(
                            [r["plan_length"] for r in level_results if r["success"]]
                        )
                    )
        ...
        print(f"Summary:")
        print(
            f"  Successful: {output['summary']['successful_levels']}/{output['summary']['total_levels']} levels"
        )
        if output["summary"]["avg_plan_length"] > 0:
            print(
                f"  Average plan length: {output['summary']['avg_plan_length']:.1f} steps"
            )

    def _train(self, levels: list[int], render: bool = False, delay: float = 0.05):
        level_results = []
    ...
    def _test(self, levels: list[int], render: bool, delay: float) -> None:
        print(
            "No reason to test Astar, the direct test results came from the training."
        )
