...
class Option:
    def __init__(self, idx: int, n_states: int, n_actions: int) -> None:
        ...
        self.theta = torch.nn.Parameter(
            torch.zeros(n_states, n_actions)
        )  # Intra policy parameters
        self.upsilon = torch.nn.Parameter(
            torch.zeros(
                n_states,
            )
    ...
    def log_pi(
        self, state_idx: int, action: int, temperature: Optional[float] = 1.0
    ) -> torch.Tensor:
...
class OptionCritic:
    def __init__(
        ...
        self.Q_U_table = torch.zeros(
            (n_states, n_options, n_actions)
        )  # Action values for state-option pair
    ...
    def get_Q_U(
        self, state_idx: int, option_idx: int, action_idx: Optional[int] = None
    ) -> torch.Tensor:
    ...
    def train(
        ...
        while not (terminated or truncated):
            ...
            self.options_improvement(
                state_idx, new_state_idx, option, action, temperature
            )
    ...
    def options_improvement(
        ...
        advantage = self.get_Q_Omega(new_state_idx, option_idx) - torch.max(
            self.get_Q_Omega(new_state_idx)
        )
    ...
    def _state_to_idx(self, state: np.ndarray, level: int) -> int:
        ...
        if state_tuple not in self.state_manager.state_to_idx_dict:
            ...
            self.state_manager.state_to_idx_dict[state_tuple] = (
                self.state_manager.n_unique_states
            )
            self.state_manager.n_unique_states += 1

            if self.state_manager.n_unique_states > self.n_states:
                logging.warning(
                    "The number of unique states exceed the expected amount of states"
                )
    ...
    def _augment_state_space(self):
        ...
        raise EnvironmentError(
            f"Number of states given: {self.n_states}. Number of unique states: {self.state_manager.n_unique_states}"
        )
